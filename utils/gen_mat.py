...
def gen_mat(model, pretrain_model, dst_path, data_loader, batch_size, patch_size, logger):
    ...
    sio.savemat(os.path.join(dst_path, 'denoised.matlab_evaluate'), {"denoised": store_data_prediction})
    sio.savemat(os.path.join(dst_path, 'clean.matlab_evaluate'), {"clean": store_data_clean})
