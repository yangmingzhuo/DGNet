...
def train(opt, epoch, model, ad_net, data_loader, optimizer, optimizer_ad, scheduler, scheduler_ad, criterion,
    ...
    for iteration, batch in enumerate(data_loader):
        ...
        if iteration % opt.print_freq == 0:
            ddp_logger_info('Train epoch: [{:d}/{:d}]\titeration: [{:d}/{:d}]\tlr={:.6f}\tad_lr={:.6f}\tl1_loss={:.4f}'
                            '\tad_loss={:.4f}\ttotal_loss={:.4f}'
                            .format(epoch, opt.nEpochs, iteration, len(data_loader), scheduler.get_lr()[0],
                                    scheduler_ad.get_lr()[0], epoch_l1_loss.avg, epoch_ad_loss.avg,
    ...
    ddp_logger_info('||==> Train epoch: [{:d}/{:d}]\tlr={:.6f}\tad_lr={:.6f}\tl1_loss={:.4f}\tad_loss={:.4f}'
                    '\ttotal_loss={:.4f}\tcost_time={:.4f}'
                    .format(epoch, opt.nEpochs, scheduler.get_lr()[0], scheduler_ad.get_lr()[0], epoch_l1_loss.avg,
                    epoch_ad_loss.avg, epoch_total_loss.avg, time.time() - t0), logger, opt.local_rank)
...
def main():
    ...
    for epoch in range(start_epoch, opt.nEpochs + 1):
        ...
        # training
        train(opt, epoch, model, ad_net, train_data_loader, optimizer, optimizer_ad, scheduler, scheduler_ad,
