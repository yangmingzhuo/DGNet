    ...
    for iteration, (noisy, target) in enumerate(data_loader):
        ...
        for i in range(prediction.shape[0]):
            for j in range(prediction.shape[1]):
                psnr.update(compare_psnr(prediction[i, j, :, :], target[i, j, :, :], data_range=1.0), 1)
...
def main():
    ...
    ddp_logger_info('Loading datasets {}, {}, {}, Batch Size: {}, Patch Size: {}'
                    ...
                    opt.local_rank)
    train_set = LoadMultiDataset_ceph_pd_2(src_path1=os.path.join(opt.data_dir, opt.data_set1, 'train'),
    ...
    val_set = LoadDataset_ceph_pd_2(src_path=os.path.join(opt.data_dir, opt.data_set_test, 'test'),
    ...
    model = ELD_UNet(in_channels=12, out_channels=12)
